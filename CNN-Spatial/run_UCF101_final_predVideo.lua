...
dirModel = '/media/cmhung/MyDisk/Dropbox (GhassanGT)/Steve/Big_and_Data/PhDResearch/Code/Activity-Recognition-with-CNN-and-RNN_Local/Models/'
			            	...
			            	local videoPathLocal = nameClass[c]..'/'..videoName
				          	...
				          	if opt.mode == 'pred' then 
				          		if groupSplit[sp].setTe:eq(videoGroup):sum() == 1 then -- testing data
					          		local predFrames = torch.Tensor(numFrameMin):zero()
